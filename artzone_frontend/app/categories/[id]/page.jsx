...
import { ChevronLeft, Search, Filter, Star, Heart, ShoppingCart, Eye, AlertCircle } from "lucide-react"
import { Input } from "@/components/ui/input"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { useCart } from "@/contexts/cart-context"

export default function CategoryArtworksPage({ params }) {
  ...
  useEffect(() => {
    const fetchCategoryArtworks = async () => {
      try {
        ...
        console.log("Category artworks data:", data)
        ...
        toast({
          title: "Error",
          description: "Failed to load category artworks",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 py-8 px-4 md:px-6 lg:px-8">
        <div className="container mx-auto max-w-7xl">
          <Button variant="ghost" className="mb-4 pl-0 hover:pl-0" onClick={() => router.push("/categories")}>
            <ChevronLeft className="mr-2 h-4 w-4" />
            Back to Categories
          </Button>

          <h1 className="text-3xl font-bold mb-2">{category.name}</h1>
          <p className="text-muted-foreground mb-8">Explore artworks in the {category.name} category</p>
